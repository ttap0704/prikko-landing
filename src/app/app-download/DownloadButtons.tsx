"use client";

import { QRCodeSVG } from "qrcode.react";
import { useEffect, useState } from "react";

const APP_STORE_URL = "https://apps.apple.com/app/id6759122744";
const GOOGLE_PLAY_URL =
  "https://play.google.com/store/apps/details?id=io.prikko.app";

function AppleIcon() {
  return (
    <svg
      width="20"
      height="24"
      viewBox="0 0 20 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      aria-label="Apple"
    >
      <path
        d="M16.52 12.62C16.5 10.14 18.04 8.82 18.12 8.76C16.74 6.78 14.62 6.52 13.88 6.5C12.08 6.32 10.34 7.58 9.42 7.58C8.5 7.58 7.08 6.52 5.54 6.54C3.58 6.56 1.76 7.72 0.76 9.5C-1.28 13.1 0.22 18.44 2.18 21.36C3.16 22.78 4.32 24.38 5.86 24.32C7.36 24.26 7.92 23.36 9.72 23.36C11.52 23.36 12.04 24.32 13.6 24.3C15.22 24.26 16.22 22.84 17.18 21.4C18.32 19.76 18.78 18.16 18.8 18.08C18.76 18.06 16.54 17.18 16.52 12.62Z"
        fill="white"
      />
      <path
        d="M13.54 4.56C14.34 3.58 14.88 2.24 14.74 0.9C13.58 0.96 12.14 1.7 11.32 2.66C10.58 3.52 9.92 4.9 10.08 6.2C11.38 6.3 12.72 5.54 13.54 4.56Z"
        fill="white"
      />
    </svg>
  );
}

function GooglePlayIcon() {
  return (
    <svg
      width="22"
      height="24"
      viewBox="0 0 22 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      aria-label="Google Play"
    >
      <path
        d="M0.65 0.65C0.25 1.05 0 1.7 0 2.55V21.45C0 22.3 0.25 22.95 0.65 23.35L0.75 23.45L11.85 12.35V12.05V11.75L0.75 0.55L0.65 0.65Z"
        fill="url(#paint0)"
      />
      <path
        d="M15.55 16.15L11.85 12.45V12.05V11.65L15.55 7.95L15.65 8.05L20.05 10.55C21.3 11.25 21.3 12.35 20.05 13.05L15.65 16.05L15.55 16.15Z"
        fill="url(#paint1)"
      />
      <path
        d="M15.65 16.05L11.85 12.05L0.65 23.35C1.05 23.75 1.75 23.8 2.55 23.35L15.65 16.05Z"
        fill="url(#paint2)"
      />
      <path
        d="M15.65 8.05L2.55 0.75C1.75 0.3 1.05 0.35 0.65 0.75L11.85 12.05L15.65 8.05Z"
        fill="url(#paint3)"
      />
      <defs>
        <linearGradient
          id="paint0"
          x1="10.55"
          y1="1.85"
          x2="-2.85"
          y2="15.25"
          gradientUnits="userSpaceOnUse"
        >
          <stop stopColor="#00A0FF" />
          <stop offset="1" stopColor="#00E886" />
        </linearGradient>
        <linearGradient
          id="paint1"
          x1="22.15"
          y1="12.05"
          x2="-0.25"
          y2="12.05"
          gradientUnits="userSpaceOnUse"
        >
          <stop stopColor="#FFE000" />
          <stop offset="1" stopColor="#FFA500" />
        </linearGradient>
        <linearGradient
          id="paint2"
          x1="13.55"
          y1="14.15"
          x2="-4.05"
          y2="31.75"
          gradientUnits="userSpaceOnUse"
        >
          <stop stopColor="#FF3A44" />
          <stop offset="1" stopColor="#C31162" />
        </linearGradient>
        <linearGradient
          id="paint3"
          x1="-1.65"
          y1="-5.05"
          x2="7.05"
          y2="3.65"
          gradientUnits="userSpaceOnUse"
        >
          <stop stopColor="#32A071" />
          <stop offset="1" stopColor="#2DA771" />
        </linearGradient>
      </defs>
    </svg>
  );
}

function QrCodeModal({ url, onClose }: { url: string; onClose: () => void }) {
  useEffect(() => {
    document.body.style.overflow = "hidden";
    const handler = (e: KeyboardEvent) => {
      if (e.key === "Escape") onClose();
    };
    document.addEventListener("keydown", handler);
    return () => {
      document.body.style.overflow = "";
      document.removeEventListener("keydown", handler);
    };
  }, [onClose]);

  return (
    <div
      role="dialog"
      aria-modal="true"
      aria-label="QR 코드 다운로드"
      className="fixed inset-0 z-50 flex items-center justify-center bg-black/50"
      onClick={onClose}
      onKeyDown={(e) => {
        if (e.key === "Escape") onClose();
      }}
    >
      {/* biome-ignore lint/a11y/noStaticElementInteractions: stops click propagation to backdrop */}
      <div
        className="w-[360px] rounded-2xl bg-white px-10 py-8 shadow-xl"
        onClick={(e) => e.stopPropagation()}
        onKeyDown={(e) => e.stopPropagation()}
      >
        <div className="flex flex-col items-center gap-4">
          <h3 className="text-lg font-semibold text-[#313131]">
            QR코드를 스캔하세요
          </h3>
          <QRCodeSVG value={url} size={200} />
          <p className="text-center text-sm leading-relaxed text-[#575757]">
            휴대폰 카메라로 QR코드를 스캔하면
            <br />
            앱을 다운로드할 수 있습니다.
          </p>
          <button
            type="button"
            onClick={onClose}
            className="mt-2 text-sm text-[#575757] underline-offset-2 hover:text-[#313131] hover:underline"
          >
            닫기
          </button>
        </div>
      </div>
    </div>
  );
}

export function DownloadButtons() {
  const [isMobile, setIsMobile] = useState(true);
  const [qrUrl, setQrUrl] = useState<string | null>(null);

  useEffect(() => {
    setIsMobile(/iPhone|iPad|iPod|Android/i.test(navigator.userAgent));
  }, []);

  const handleClick =
    (url: string) => (e: React.MouseEvent<HTMLAnchorElement>) => {
      if (!isMobile) {
        e.preventDefault();
        setQrUrl(url);
      }
    };

  return (
    <>
      <div className="mt-6 flex gap-4 md:mt-10">
        <a
          href={APP_STORE_URL}
          target="_blank"
          rel="noopener noreferrer"
          onClick={handleClick(APP_STORE_URL)}
          className="inline-flex h-[54px] items-center gap-3 rounded-lg bg-black px-5 transition-opacity hover:opacity-80 md:h-[60px] md:px-6"
        >
          <AppleIcon />
          <div className="flex flex-col items-start">
            <span className="text-[10px] leading-tight text-white/90">
              Download on the
            </span>
            <span className="text-[16px] font-semibold leading-tight text-white md:text-[18px]">
              App Store
            </span>
          </div>
        </a>

        <a
          href={GOOGLE_PLAY_URL}
          target="_blank"
          rel="noopener noreferrer"
          onClick={handleClick(GOOGLE_PLAY_URL)}
          className="inline-flex h-[54px] items-center gap-3 rounded-lg bg-black px-5 transition-opacity hover:opacity-80 md:h-[60px] md:px-6"
        >
          <GooglePlayIcon />
          <div className="flex flex-col items-start">
            <span className="text-[10px] leading-tight text-white/90 uppercase">
              Get it on
            </span>
            <span className="text-[16px] font-semibold leading-tight text-white md:text-[18px]">
              Google Play
            </span>
          </div>
        </a>
      </div>

      {qrUrl && <QrCodeModal url={qrUrl} onClose={() => setQrUrl(null)} />}
    </>
  );
}
